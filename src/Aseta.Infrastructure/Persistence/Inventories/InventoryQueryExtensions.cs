...
internal static class InventoryQueryExtensions
{
    ...
    {
        if (!createdAtFrom.HasValue) return query;
    ...
    {
        if (!createdAtTo.HasValue) return query;
    ...
    {
        if (tagIds.Count == 0) return query;
    ...
    {
        if (categoryIds.Count == 0) return query;
    ...
    {
        if (!isPublic.HasValue) return query;
    ...
    {
        if (!minItemsCount.HasValue) return query;
    ...
    {
        if (!maxItemsCount.HasValue) return query;
